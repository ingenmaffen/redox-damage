...
pub fn sub(cpu: &mut CPU, source: InstructionSourceTarget) {
    ...
    handle_sub(cpu, subtracted_value);
    cpu.pc += 1;
}
...
fn handle_sub(cpu: &mut CPU, subtracted_value: u8) {
    set_sub_flags(cpu, subtracted_value);
...
fn set_sub_flags(cpu: &mut CPU, subtracted_value: u8) {
    // Flags: Z, 1, H, C
